...
(function() {
    ...
    **/
    this.insertLines = function(row, lines) {
        ...
        var oldLen = this.getValue().length;
        ...
        var easySync = this.createEasySyncDelta(oldLen, delta);
        delta.easySync = easySync;
    ...
    **/
    ...
    this.toAbsolutePosition = function(position) {
    	var pos = 0;
    	var lineCount = 0;
    	...
    	pos+=position.column;
    	return {position:pos, lines:lineCount};
    };
    
    this.getCursorForPosition = function(position) {
    	var row = 0;
    	var col = position
    	for(var i = 0; i < this.$lines.length; i++) {
    		if (this.$lines[i].length < col){
    			col = col - this.$lines[i].length - 1;
    			++row
    		}else {
    			break;
    		}
    ...
    this.createEasySyncDelta = function(oldLen, delta) {
    	var assem = Changeset.mergingOpAssembler();
        var op = Changeset.newOp();
        var bank = Changeset.stringAssembler();
        var oldPos = 0;
        var newLen = 0;
        //console.log("old delta:"+JSON.stringify(delta));
        //generate leading keep
        if (delta.range.start.row > 0 || delta.range.start.column > 0) {
        	var keepPos = this.toAbsolutePosition(delta.range.start);
        	var keepOp = Changeset.newOp();
        	keepOp.opcode = '=';
        	keepOp.chars = keepPos.position;
        	keepOp.lines = keepPos.lines;
            assem.append(keepOp);
            oldPos += op.chars;
            newLen += op.chars;
        } 
        ...
        if (delta.action == "insertLines"){
        	...
        	for(var i = 0; i < delta.lines.length; i++) {
        		bank.append(delta.lines[i]);
        		chars+=delta.lines[i].length;
        		bank.append(nl);
        		chars+=1;
        	}
            op.chars = chars;
            op.lines = delta.lines.length;
            assem.append(op);
            newLen += op.chars;
        } else if (delta.action == "insertText") {
        	op.opcode = '+';
        	bank.append(delta.text);
            op.chars = delta.text.length;
            op.lines = delta.text.match(/\n*$/)[0].length;
            assem.append(op);
            newLen += op.chars;     	
        } else if (delta.action == "removeLines") {
        	op.opcode = '-';
        	...
        	op.opcode = '-';
        	var startPos = this.toAbsolutePosition(delta.range.start);
        	var endPos = this.toAbsolutePosition(delta.range.end);        	
        	op.chars = delta.text.length;
            op.lines = delta.text.match(/\n*$/)[0].length;
            assem.append(op);
            oldPos += op.chars;
        ...
        assem.endDocument();
        var cs = Changeset.pack(oldLen, newLen, assem.toString(),
                bank.toString());
        //console.log("change set:"+JSON.stringify(Changeset.unpack(cs)));
        return cs;
    };
