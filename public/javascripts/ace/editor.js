...
(function(){
    ...
    this.applyChangeset = function(cs, markerId, clientOrder) {
    	...
    	while (csIter.hasNext()) {
    	    ...
    	    switch(op.opcode) {
    	    	case '+': 
    	    		var text = bankIter.take(op.chars);
    	    		var cursor = this.session.doc.getCursorForPosition(cursorPos);
    	    		this.session.insert(cursor, text);
    	    		cursorPos += op.chars;
    	    		break;
    	    	case '-': 
    	    		var startCursor = this.session.doc.getCursorForPosition(cursorPos);
    	    		var endCursor = this.session.doc.getCursorForPosition(cursorPos+op.chars);
    	    		var range = new Range(0, 0, 0, 0);
    	    		range.start.row = startCursor.row;
    	    		range.start.column = startCursor.column;
    	    		range.end.row = endCursor.row;
    	    		range.end.column = endCursor.column;
    	...
    	var cursor = this.session.doc.getCursorForPosition(cursorPos);
    	var mrange = new Range(0, 0, 0, 0);
		mrange.start.row = cursor.row;
		mrange.start.column = cursor.column;
		mrange.end.row = cursor.row;
		mrange.end.column = cursor.column+1;
    	var mid = this.session.addMarker(mrange, "cursor remote"+clientOrder, "line");
    	return mid;  	
    };
    
    
    this.showChangesetCursor = function(cs, markerId, clientOrder) {
    	var unpacked = Changeset.unpack(cs);
    	var csIter = Changeset.opIterator(unpacked.ops);
    	var bankIter = Changeset.stringIterator(unpacked.charBank);
    	var cursorPos = 0;
