...
var EditSession = function(text, mode, sid) {
	this.$sessionId = -1;
    this.$modified = true;
    this.$breakpoints = [];
    this.$frontMarkers = {};
...
(function() {
    ...
    **/
    this.insert = function(position, text) {
    	...
    	var textLineCount = lines.length - 1;
    	for (var i = 0; i < lines.length; i++) {
    		var currLine = lines[i];
    		//console.log("current Line length:"+currLine.length+", wrap limit: "+this.$wrapLimit);
            var wrappedCount = Math.floor(currLine.length / this.$wrapLimit);
            //console.log("wrapcount:"+wrappedCount);
            textLineCount += wrappedCount;
        }
    	
    	var docLineCount = this.doc.getLength();
    	for (var i = 0; i < this.doc.getLength(); i++) {
    		var docLine = this.doc.getLine(i);
    		
            var wrappedCount = Math.floor(docLine.length / this.$wrapLimit);
            //console.log("doc line:"+docLine+":end of line");
            docLineCount += wrappedCount;
        }
    	
        var newlines= docLineCount + textLineCount;
        //console.log("doc length:"+ docLineCount +", lines from new text:"+textLineCount+", session lines count:"+newlines);
    ...
    this.setMaxLines = function(maxLines) {
    ...
    **/
    ...
    //sets the session id
    //modified by tim for collaborative editing
    this.setSessionId = function(sid) {
    	//console.log("desired session id:"+sid);
        this.$sessionId = sid;
        this.selection.setSessionId(sid);
        //console.log("after setting current session id:"+this.$sessionId);
    };
